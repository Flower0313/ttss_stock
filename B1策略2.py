...
from (
	...
	join (select ts_code from df_a_tu_stock_financial_df where trade_date='2025-11-04' and circ_mv>=1000000) c on a.ts_code=c.ts_code
	where a.trade_date>='2025-05-01' and industry<>'房产服务' and name='济川药业'
) a
"""
...
def find_local_highs(stock_df):
    ...
    for i in range(3, n):
        ...
        bullish_condition = close.iloc[i] > close.iloc[i - 1] #(close.iloc[i] > close.iloc[i - 1]) & (close.iloc[i] > open.iloc[i])
...
# ========== 最终筛选 ==========
result = df_all[
    (df_all['amplitude'] <= 7) &  # 振幅 ≤ 7
    #(df_all['up_down_rate'].between(-1.8, 2)) &                   # 涨跌幅 -2% ~ 2%（可选）
    (df_all['prev_high_vol'].notna()) &  # 有前高
    (df_all['deal_vol'] < df_all['prev_high_vol']) &  # 成交量 < 前高成交量
    (df_all['J'] <=13) &  # J < 13
    (df_all['white_line'] >= df_all['yellow_line']) &  # 白线 ≥ 黄线
    # (df_all['closing_price'] < df_all['white_line']) &
    (df_all['closing_price'] >= df_all['yellow_line']) &  # 收盘价 ≥ 黄线
    # (df_all['closing_price'] <= df_all['white_line']) &         # 收盘价 ≤ 白线（可选）
    # (df_all['closing_price'] >= df_all['BBI']) &                # 收盘价 ≥ BBI（可选）
    # (abs(df_all['closing_price'] - df_all['white_line']) / df_all['white_line'] <= 0.02) & # 白线附近（可选）
    # (~df_all['drop_signal']) &                                  # 没有大跌信号（可选）
    (df_all['ds'] == '2025-11-25')
    ][[
    'ds', 'code', 'opening_price', 'closing_price', 'up_down_rate',
    'amplitude', 'deal_vol', 'prev_high_price', 'prev_high_vol','prev_high_date',
    'J', 'white_line', 'yellow_line','industry'
]]
...
# con.execute("""
# INSERT INTO df_a_stock_day_b1_df
# SELECT code, ds
# FROM tmp_result
# """)
con.commit()
# ========== 导出到 Excel ==========
# output_file = "stock_signal_b1.xlsx"
